...
if ($what == 'inn') {
    ...
    if ($p == '3') {
        ...
        $dt = get_data("SELECT nama, username,kode,id FROM skp_skpd where username = '$usr' and  password = '$pwd'");
        if(count($dt) > 0){
            $_SESSION['_namaSkpd'] = $dt['nama'];
            $_SESSION['_userSkpd'] = $dt['username'];
            $_SESSION['_kodeSkpd'] = $dt['kode'];
            $_SESSION['_idSkpd'] = $dt['id'];
            $_SESSION['_menu'] = $p;
            echo 'suk___admindata.php';
        }else{
            echo 'gal';
