...
if ($act == 'jabatan_simpan') {
    ...
    if ($id > 0) {
		if($jabatan=="Jabatan Struktural"){
			...
			exec_query("update skp_jabatan set unit_kerja=".$skpd.", nama_jabatan='".$jab_bkn."', parent=".$induk.", kode_jabatan='" . $kode . "', jabatan='".$jabatan."', unit_organisasi=".$unitorgnisasi." where idjab=" . $id . "");
		}
        echo 'success__';
    } else {
		if($jabatan=="Jabatan Struktural"){
    ...
    if (is_numeric($id)) {
        ...
        print_r($ress);
		echo '__'.$i.'__'.$nmjab.'__'.$pecah[0].'__'.$pecah[1];
	} else {
        echo 'error';
    }
