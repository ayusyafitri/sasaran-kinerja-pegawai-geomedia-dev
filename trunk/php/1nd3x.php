    ...
    if (count($cek) > 0) {
        ...
        if (count($h) > 0) {
            ...
            if (count($dt) > 0) {
                ...
                $goto = "gal___<font color='red'>Thi's weird there is no data &  no acces login !!!</font>";
                $uu = FALSE;
            }
        }
        if ($uu) {
            $max = get_maxid('log_id', 'skp_logs');
            exec_query("insert into skp_logs(log_id,log_action,log_level,log_user,log_datetime) values(" . $max . ",'login','" . $_SESSION['_menu'] . "','" . $_SESSION['_id'] . "',NOW())");
            echo $goto;
        } else {
            echo "gal___<font color='red'>Gagal Login !!!</font>";
        }
    } else {
